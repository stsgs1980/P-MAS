...
### Rule 12. Запрет Rebase в Sandbox

| Команда | Статус |
|---|---|
| `git pull --rebase` | ЗАПРЕЩЕНО |
| `git rebase` (любая форма) | ЗАПРЕЩЕНО |
| `git pull` / `git pull --no-rebase` | РАЗРЕШЕНО |

Причина: rebase вызывает конфликт → sandbox блокирует ВСЕ инструменты навсегда.

### Rule 13. Пять проверок перед объявлением потери данных

| # | Проверка | Команда |
|---|---|---|
| 1 | Локальный git log | `git log --oneline -20` |
| 2 | Удалённый репозиторий | `git log origin/main --oneline -20` |
| 3 | Незафиксированные изменения | `git status` |
| 4 | Stash | `git stash list` |
| 5 | Физические файлы | `ls -la` |

НЕВЕРНО объявлять данные потерянными без прохождения всех 5 проверок.

### Rule 14. Контроль перед коммитом

Перед `git commit` обязательно выполнить `git diff` и проверить:
- Изменены ТОЛЬКО запрошенные файлы
- Нет самовольных новых файлов или структур
- Нет удаления файлов без явного запроса
